...
//音频播放
JNIEXPORT void JNICALL Java_com_sun_eg15_VideoPlayer_audioPlayer
...
int result=0;
//打开音频文件
result=avformat_open_input(&pFormatCtx, input_cstr, NULL, NULL);
if ( result != 0) {
LOGI("%d", result);//-13是没权限
LOGI("%s", "无法打开音频文件 ");
return;
}
//获取输入文件信息
...
jclass cls = (*env)->FindClass(env, "com/sun/eg15/AudioUtil");
//jmethodID
