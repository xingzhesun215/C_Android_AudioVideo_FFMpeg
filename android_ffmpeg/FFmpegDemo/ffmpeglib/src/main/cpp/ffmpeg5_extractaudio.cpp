...
// 使用FFmpeg从视频中抽取音频
void extractAudio(const char *srcFile) {
...
//    ret = avformat_open_input(&fmt_ctx, "http://vfx.mtime.cn/Video/2019/03/17/mp4/190317150237409904.mp4", NULL, NULL);
    ret = avformat_open_input(&fmt_ctx, srcFile, NULL, NULL);

    // 检查打开输入文件是否成功
    if (ret < 0) {
        LOGE("cant open file，error message = %s\n", av_err2str(ret));
        return;
    }
    LOGE("打开输入文件成功");

    // 打开输出文件
    FILE *dst_fd = fopen("/sdcard/test_av/111.aac", "wb"); // w 写入  b 二进制文件
    ...
    LOGE("Audio Stream Index = %d", audio_index);

    // 检查发现音频流的结果
    if (audio_index < 0) {
        LOGE(NULL, AV_LOG_ERROR, "Can't find Best Audio Stream!\n");
...
Java_com_sun_ffmpeglib_FFmpeg5ExtractAudio_extractAudio(JNIEnv *env, jclass clazz,
                                                        jstring file_path) {
    const char *path = env->GetStringUTFChars(file_path, nullptr);
    LOGE("the extract audio file path is %s", path);
    extractAudio(path);
}
