...
// 使用FFmpeg从视频中抽取视频
void extractVideo(const char* filePath) {
    ...
    // 检查打开输入文件是否成功
    if (ret < 0) {
        LOGE("cant open file，error message = %s", av_err2str(ret));
        return;
    }
    LOGE("打开输入文件成功");
   // 打开输出文件
    FILE* dst_fd = fopen("/sdcard/test_av/333.h264", "wb");  // w 写入  b 二进制文件
    ...
    pkt.data = NULL;
    pkt.size = 0;
    LOGE("Video Stream Index1 = %d", video_index);
...
Java_com_sun_ffmpeglib_FFmpeg6ExtractVideo_extractVideo(JNIEnv *env, jclass clazz,
                                                        jstring file_path) {
    const char *path = env->GetStringUTFChars(file_path, nullptr);
    LOGE("the extract video file path is %s", path);
    extractVideo(path);
}
